...
sort3_riscv_asm_i32 = [
    "lw  x2  1 x0", # @0  mov 0x4(%0), %%eax  x2 = mem[0x4]
    "lw  x3  2 x0", # @4  mov 0x8(%0), %%ecx  x3 = mem[0x8]
    "sub x1 x2 x3", # @8  cmp %%eax, %%ecx    x1 = x2 - x3  -|- >0 if x2 > x3; <=0 if x2 <= x3
    "add x4 x0 x2", # @12 mov %%eax, %%edx    x4 = x2 + 0
    "bge x1 x0 24", # @16 cmovl %%ecx, %%edx  jump if ( x2-x3 ) >= 0 [x2 greater than x3]
    "add x4 x0 x3", # @20                     move if ( x2-x3 ) <  0 [x2 less    than x3]
    ...
    "blt x0 x1 60", # @52 cmovle %%r8d, %%eax jump if ( x5-x2 ) >  0 [x5 greater than x2]
    "add x2 x0 x5", # @56                     move if ( x5-x2 ) <= 0 [x5 less    than x2]
    "sw  x2  2 x0", # @60 mov %%eax, 0x8(%0)  mem[0x8] = x2
    "sub x1 x3 x4", # @64 cmp %%ecx, %%edx    x1 = x3 - x4  -|- >0 if x3 > x4; <=0 if x3 <= x4
    "blt x0 x1 76", # @68 cmovle %%edx, %%r8d jump if ( x3-x4 ) >  0 [x3 greater than x4]
    "add x5 x0 x4", # @72                     move if ( x3-x4 ) <= 0 [x3 less    than x4]
    "sw  x5  0 x0", # @76 mov %%r8d, (%0)     mem[0x0] = x5
...
def process_riscv_asm(prog):
    ...
    for insn in prog:
        ...
        riscv_asm_split.append([op, *operands])

    # convert the absolute offsets to relative offsets
    # in branching instructions to make them covariant to the current pc
    riscv_asm_rel = []
    for insn in riscv_asm_split:
        op, operands = insn[0], insn[1:]
        if op.startswith('b'): #bge or blt, the last operand is the offset. always 8
            operands = [operands[0], operands[1], 8]
        riscv_asm_rel.append([op, *operands])
    return riscv_asm_split, riscv_asm_rel

sort3_riscv_asm_split, sort3_riscv_asm_rel = process_riscv_asm(sort3_riscv_asm)
sort3_riscv_asm_split_i32, sort3_riscv_asm_rel_i32 = process_riscv_asm(sort3_riscv_asm_i32)
...
def convert_riscv_asm_to_program(riscv_asm_rel):
    program = []
    for insn in riscv_asm_rel:
        op, operands = insn[0], insn[1:]
        program.append((op.upper(), operands))
    return program

program = convert_riscv_asm_to_program(sort3_riscv_asm_rel)
program_i32 = convert_riscv_asm_to_program(sort3_riscv_asm_rel_i32)

# test case 2 -- test a pseudo instruction machine 
def _test_pseudo_asm_machine(machine_cls, program):
    m = machine_cls(1024)
    # input starts at 0
    for i in range(test_cases.shape[0]):
        inp, out = test_cases[i,0], test_cases[i,1]
        ...
        # check if the output is correct
        assert np.array_equal(outp, out), f"Test case {i} failed: expected {out}, got {outp}"
    print(f"Test case 2.{machine_cls.__class__} passed")

def test_pseudo_asm_machine():
    # test the pseudo asm machine
    _test_pseudo_asm_machine(pseudo_asm_machine, program=program)
    # test the multi machine
    _test_pseudo_asm_machine(pseudo_asm_machine_32, program=program_i32)

# test case 3 -- test a multi machine
def _test_multi_machine(mode, program):
    m = multi_machine(1024, 13, test_cases[:,0,:], mode=mode)
    ...
    print(f"Test case 3.{mode} passed")

def test_multi_machine():
    # test the multi machine with the default mode
    _test_multi_machine('u8', program=program)
    # test the multi machine with the i32 mode
    _test_multi_machine('i32', program=program_i32)
...
def _test_x86_to_riscv(mode, target):
    riscv_action_space = []
    for x86_action, x86_operands in x86_action_space.values():
        riscv_action_space.extend(x86_to_riscv(x86_action, x86_operands, 6, mode=mode))
    ...
    print(f"Test case 4.{mode} passed")

def test_x86_to_riscv():
    # test the x86 to riscv translation with u8 mode
    _test_x86_to_riscv('u8', sort3_riscv_asm_rel)
    # test the x86 to riscv translation with i32 mode
    _test_x86_to_riscv('i32', sort3_riscv_asm_rel_i32)

# test case 5 -- see if translation of x86 to riscv produces the expected output
def _test_x86_to_riscv_translation_correct(mode, x86_target, riscv_target):
    action_space_storage = x86ActionSpaceStorage(6,5, mode=mode)
    # map x86 instructions to integers
    reverse_action_loopup = {tuple(a): i for i, a in action_space_storage.actions.items()}
    translated = []
    for x86insn in x86_target:
    ...
    for translated, og in zip(translated, riscv_target):
    ...
    print(f"Test case 5.{mode} passed")

def test_x86_to_riscv_translation_correct():
    # test the x86 to riscv translation with u8 mode
    _test_x86_to_riscv_translation_correct('u8', sort3_x86_asm_rel, sort3_riscv_asm_rel)
    # test the x86 to riscv translation with i32 mode
    _test_x86_to_riscv_translation_correct('i32', sort3_x86_asm_rel, sort3_riscv_asm_rel_i32)
